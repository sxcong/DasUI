...
DownsampleWidget::DownsampleWidget(QWidget* parent)
    : QWidget(parent)
{
    setupUi();
    setupPlot();

    connect(m_plot, &QCustomPlot::mousePress, this, &DownsampleWidget::MyMouseMoveEvent);
...
void DownsampleWidget::setupUi()
{
    ...
    toolbar->setObjectName("downsampleToolbar");
    toolbar->setFixedHeight(38);
    toolbar->setStyleSheet(R"(
        QWidget#downsampleToolbar { background: #f6f7f9; border-bottom: 1px solid #d7dce2; }
    ...
    openButton->setToolTip("打开一个或多个降采样二进制文件");
    connect(openButton, &QToolButton::clicked, this, &DownsampleWidget::openFiles);

    auto* fitButton = new QToolButton(toolbar);
    fitButton->setText("全览");
    fitButton->setToolTip("显示全部文件时间范围");
    connect(fitButton, &QToolButton::clicked, this, &DownsampleWidget::fitToAll);
...
void DownsampleWidget::setupPlot()
{
    ...
    m_title = new QCPTextElement(m_plot, "降采样原始数据图", QFont("Microsoft YaHei", 13, QFont::Bold));
...
void DownsampleWidget::MyMouseMoveEvent(QMouseEvent *event)
{
...
void DownsampleWidget::setFiles(const QStringList& files)
{
    m_files = files;
...
void DownsampleWidget::setSampleIntervalSeconds(double seconds)
{
    if (seconds > 0.0) {
...
void DownsampleWidget::setDepthIntervalMeters(double meters)
{
    if (meters > 0.0) {
...
void DownsampleWidget::openFiles()
{
...
void DownsampleWidget::fitToAll()
{
    if (m_segments.isEmpty()) {
        return;
    }
...
void DownsampleWidget::scheduleRangeLoad()
{
    m_rangeLoadTimer.start();
}

void DownsampleWidget::rebuildSegments()
{
    ...
    m_title->setText(QString("降采样原始数据图 | %1")
                         .arg(m_segments.first().startTime.toString("yyyy-MM-dd HH:mm:ss.zzz")));
    scheduleRangeLoad();
}

bool DownsampleWidget::readBinaryHeader(const QString& filePath, int& rows, int& cols) const
{
    QFile file(filePath);
...
bool DownsampleWidget::readBinaryData(const QString& filePath, Segment& segment) const
{
    QFile file(filePath);
    if (!file.open(QIODevice::ReadOnly)) {
        return false;
...
QDateTime DownsampleWidget::parseStartTime(const QString& filePath) const
{
...
bool DownsampleWidget::ensureDataForRange(double lower, double upper)
{
    bool changed = false;
...
void DownsampleWidget::refreshMapForRange(double lower, double upper)
{
    if (m_segments.isEmpty()) {
        return;
    }
...
void DownsampleWidget::smoothSegmentBoundaries(QCPColorMapData* mapData, int timeCells, int depthCols,
...
void DownsampleWidget::updateStatus()
{
    int loadedCount = 0;
    int failedCount = 0;
    ...
    QString status = QString("%1 个文件，已缓存 %2 个，%3 - %4")
        .arg(m_segments.size())
        .arg(loadedCount)
...
double DownsampleWidget::fileDurationSeconds(int rows) const
{
