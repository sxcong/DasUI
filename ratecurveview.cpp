    ...
    QWidget(parent)
{
    ...
    verticalLayout->addWidget(mPlot);
...
//    m_axisRange->hide();
    ...
    if(m_Traser == nullptr)
    {
        m_Traser = new PointTracer(mPlot, mPlot->graph(0), CustomerTracer);
    }
...
RateCurveView::~RateCurveView()
{

}
void RateCurveView::curveInit()
{
    //设置坐标轴颜色
    ...
    mPlot->xAxis->setTickLabelColor(QColor("#FFFFFF")); mPlot->yAxis->setLabelColor(QColor("#FFFFFF"));
...
//        mPlot->xAxis->grid()->setSubGridVisible(true);
    ...
    mGraph1->setPen(QPen(color.lighter(200)));
...
//    connect(mPlot,SIGNAL(mouseMove(QMouseEvent*)),this,SLOT(paintbigpoint(QMouseEvent *)));
     connect(mPlot, &QCustomPlot::mouseMove, this, &RateCurveView::MyMouseMoveEvent);
...
//    connect(mPlot, &QCustomPlot::axisDoubleClick, this, &RateCurveView::slot_axisDoubleClick);
...
void RateCurveView::addData1(double keys, double values)
{
    mGraph1->addData(keys,values);
}


void RateCurveView::updateLine1(const QVector<double> &keys, const QVector<double> &values)
{
    mGraph1->setData(keys,values);
    mPlot->replot();
}

void RateCurveView::updateLine(const QList<QPointF> &list)
{
    QVector<double> keys;
...
void RateCurveView::updateLine(const QVector<QPointF> &list)
{
    updateLine(list.toList());
}

void RateCurveView::setRangeX(const int nMin, const int nMax)
{
    m_x1 = nMin;
    m_x2 = nMax;
...
void RateCurveView::setRangeY(const double nMin, const double nMax)
{
    m_y1 = nMin;
    m_y2 = nMax;
...
void RateCurveView::rescaleAllline()
{
    mPlot->rescaleAxes();
    mPlot->replot();
}

void RateCurveView::clearAllline()
{
    if(nullptr != mGraph1)
    {
...
void RateCurveView::MyMouseMoveEvent(QMouseEvent *event)
{
    ...
    if (dx <= 100 && y_posval != 0)
    {
        m_Traser->setVisible(true);
//        qDebug()<<y_posval<<x_posval;
        ...
        if(m_type == 0)
        {
            m_Traser->setText2(QString("深度:%1m\n比例:%2%\n流量:%3").
                               ...
                               arg(QString::number(m_flowRate* y_posval/100, 'f', 2)));
        }
...
void RateCurveView::contextMenuRequest(const QPoint &pos)
{
    Q_UNUSED(pos);
...
void RateCurveView::selectionChanged()
{
    /*
...
void RateCurveView::xAxisChanged(const QCPRange &newRange, const QCPRange &oldRange)
{
    if(newRange.lower < 0 )
    {
...
void RateCurveView::yAxisChanged(const QCPRange &newRange, const QCPRange &oldRange)
{
...
void RateCurveView::paintbigpoint(QMouseEvent *event)
{
...
void RateCurveView::slot_axisDoubleClick(QCPAxis *axis, QCPAxis::SelectablePart part, QMouseEvent *event)
{
    // 当前鼠标位置（像素坐标）
    int x_pos = event->pos().x();
    int y_pos = event->pos().y();
...
void RateCurveView::setupSimpleItemDemo(QCustomPlot *customPlot)
{
...
void RateCurveView::createNewLine()
{
    QPen pen;
...
void RateCurveView::updateSymbolicLines(QList<QList<QPointF> > &linePoints, int alarmType)
{
    QPen pen;
