...
FbeEnergyWidget::FbeEnergyWidget(QWidget* parent)
    : QWidget(parent)
{
    setupUi();
    setupPlot();

    connect(m_plot, &QCustomPlot::mousePress, this, &FbeEnergyWidget::MyMouseMoveEvent);
...
void FbeEnergyWidget::setupUi()
{
    ...
    toolbar->setObjectName("fbeEnergyToolbar");
    toolbar->setFixedHeight(38);
    toolbar->setStyleSheet(R"(
        QWidget#fbeEnergyToolbar {
            background: #f6f7f9;
    ...
    openButton->setToolTip("打开一个或多个 FBE 二进制文件");
    connect(openButton, &QToolButton::clicked, this, &FbeEnergyWidget::openFiles);

    auto* reloadButton = new QToolButton(toolbar);
    reloadButton->setText("刷新");
    reloadButton->setToolTip("重新加载当前显示范围内的数据");
    connect(reloadButton, &QToolButton::clicked, this, &FbeEnergyWidget::reloadLoadedFiles);

    auto* fitButton = new QToolButton(toolbar);
    fitButton->setText("全览");
    fitButton->setToolTip("显示全部文件时间范围");
    connect(fitButton, &QToolButton::clicked, this, &FbeEnergyWidget::fitToAll);
    ...
    m_loadingLabel = new QLabel("正在加载 FBE 数据...", m_plot);
...
void FbeEnergyWidget::setupPlot()
{
    ...
    m_title = new QCPTextElement(m_plot, "FBE 分频能量瀑布图", QFont("Microsoft YaHei", 13, QFont::Bold));
...
void FbeEnergyWidget::MyMouseMoveEvent(QMouseEvent *event)
{
...
void FbeEnergyWidget::setFiles(const QStringList& files)
{
    m_files = files;
...
void FbeEnergyWidget::addFiles(const QStringList& files)
{
...
void FbeEnergyWidget::setSampleIntervalSeconds(double seconds)
{
    if (seconds > 0.0) {
...
void FbeEnergyWidget::setDepthIntervalMeters(double meters)
{
    if (meters > 0.0) {
...
void FbeEnergyWidget::setVisibleSpanSeconds(double seconds)
{
    if (seconds > 0.0) {
...
void FbeEnergyWidget::openFiles()
{
...
void FbeEnergyWidget::reloadLoadedFiles()
{
...
void FbeEnergyWidget::scheduleRangeLoad()
{
    m_rangeLoadTimer.start();
}

void FbeEnergyWidget::rebuildSegments()
{
    ...
    m_title->setText(QString("FBE 分频能量瀑布图 | %1").arg(titleTime));
    scheduleRangeLoad();
}

bool FbeEnergyWidget::readBinaryHeader(const QString& filePath, int& rows, int& cols) const
{
    QFile file(filePath);
...
bool FbeEnergyWidget::readBinaryData(const QString& filePath, Segment& segment) const
{
    QFile file(filePath);
    if (!file.open(QIODevice::ReadOnly)) {
        return false;
...
QDateTime FbeEnergyWidget::parseStartTime(const QString& filePath) const
{
...
bool FbeEnergyWidget::ensureDataForRange(double lower, double upper)
{
    bool changed = false;
    showLoading(true);
...
void FbeEnergyWidget::refreshMapForRange(double lower, double upper)
{
    if (m_segments.isEmpty()) {
        return;
    }
...
void FbeEnergyWidget::smoothSegmentBoundaries(QCPColorMapData* mapData,
                                             int timeCells,
                                             int depthCols,
                                             double dataStart,
                                             double dataEnd)
...
void FbeEnergyWidget::updateStatus()
{
    int loadedCount = 0;
    int failedCount = 0;
...
void FbeEnergyWidget::showLoading(bool visible)
{
    if (!m_loadingLabel) {
        return;
    }
...
double FbeEnergyWidget::fileDurationSeconds(int rows) const
{
